...
function StepButton({ label, onClick, disabled }: { label: string; onClick: () => void; disabled: boolean }): JSX.Element {
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="rounded-md border border-white/10 bg-white/[0.04] px-1.5 py-0.5 text-[11px] font-bold text-zinc-300 transition-colors duration-150 hover:border-white/25 hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-30"
    >
      {label}
    </button>
  );
  ...
  return (
    <div className={`panel-inset p-2.5 ${hasLost ? "opacity-60" : ""}`}>
      <div className="mb-1.5 flex items-center gap-1.5">
        ...
        {hasPriority && <span className="chip border-emerald-400/40 text-emerald-300">priority</span>}
