  ...
  return (
    <button
      ...
      className={`flex-1 rounded-md px-2 py-1.5 text-xs font-semibold transition-all duration-150 ${
        active ? "bg-emerald-600 text-white shadow-card" : "bg-white/[0.04] text-zinc-400 hover:bg-white/10"
      }`}
    >
        ...
        <div className="panel-inset mb-3 p-3">
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              <div className="truncate text-sm font-bold text-emerald-300">{cube.name}</div>
              <div className="text-xs text-zinc-500">{cube.cardCount} cards resolved</div>
            </div>
            <svg viewBox="0 0 24 24" className="h-6 w-6 shrink-0 fill-emerald-500/70"><path d="M12 1.5 3 6.75v10.5L12 22.5l9-5.25V6.75L12 1.5Z" /></svg>
          </div>
      ...
      {isHost ? (
        <>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="flex-1 truncate text-[11px] text-zinc-600">
        ...
        !cube && (
          <div className="flex flex-col items-center gap-1.5 rounded-lg border border-dashed border-white/10 py-6 text-center">
            <svg viewBox="0 0 24 24" className="h-6 w-6 fill-zinc-700"><path d="M12 1.5 3 6.75v10.5L12 22.5l9-5.25V6.75L12 1.5Z" /></svg>
            <span className="text-xs text-zinc-500">Waiting for the host to upload a cube…</span>
          </div>
        )
      )}
    </section>
    ...
    <div className="mx-auto max-w-6xl animate-fade-in p-4 md:p-6">
      ...
      <header className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <svg viewBox="0 0 24 24" className="h-8 w-8 fill-emerald-400"><path d="M12 1.5 3 6.75v10.5L12 22.5l9-5.25V6.75L12 1.5Z" /></svg>
          <div>
          ...
          <ul className="space-y-1.5">
            {room.players.map((p) => (
              <li key={p.id} className="flex items-center gap-2 rounded-lg bg-white/[0.03] px-3 py-2">
                ...
                />
                <span className={`flex-1 truncate text-sm font-semibold ${p.id === me.playerId ? "text-emerald-300" : "text-zinc-200"}`}>
                  {p.name}
