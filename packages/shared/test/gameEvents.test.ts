...
describe("v10 existing effect behavior unchanged (spot checks)", () => {
  ...
  it("counterTarget through a spell-effect entry counters a spell on the stack", () => {
    const g = newGame();
    put(g, "p1", "hand", "bear", "bear1");
    ...
    // Counterspell resolves: card to graveyard, effect entry on the stack.
    s = applyAction(s, "p2", { type: "resolveTopOfStack" }, 0, ctx);
    expect(player(s, "p2").zones.graveyard.some((c) => c.instanceId === "csp1")).toBe(true);
    expect(s.stack[s.stack.length - 1]!.isTrigger).toBe(true);
    // Controller resolves the effect entry, targeting the bear on the stack.
