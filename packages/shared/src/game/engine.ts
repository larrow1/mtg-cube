...
export function applyAction(
  ...
  const logs: string[] = [];
  // v9: track consecutive priority passes since the stack's top last changed
  // (CR 117.4). Anything that pushes a new object onto the stack — a cast,
  // any pushTriggers call site, the admin spawnCard-to-stack path — voids a
  // pending pass sequence; this single diff check covers all of them.
  ...
  switch (action.type) {
    ...
    case "resolveTopOfStack": {
      ...
      // v9 (CR 117.5): the active player receives priority after a resolution.
      s.priorityPasses = 0;
      ...
      if (!card.isToken && typeLine !== undefined && /\b(?:Instant|Sorcery)\b/i.test(typeLine)) {
        ...
        if (effects && effects.length > 0) {
          // v9: the spell's effect applies immediately as part of THIS
...
/**
 * v9: resolve the TargetRef for an effect at resolution time — shared by
...
function applyMoveCard(
  ...
  switch (to) {
    case "stack":
      ...
      s.stack.push(card);
      // v9: the caster retains priority after casting (CR 117.3b/601.2i) —
